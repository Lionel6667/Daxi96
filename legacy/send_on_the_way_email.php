...
try {
    ...
    $mail->isHTML(true);
    $mail->CharSet = 'UTF-8';
    $mail->Subject = 'Votre chauffeur est en route !';
...
<body style="margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Helvetica Neue,Arial,sans-serif;background-color:#f0f2f5">
                ...
                <table width="600" cellpadding="0" cellspacing="0" style="background-color:#fff;border-radius:20px;overflow:hidden;box-shadow:0 10px 40px rgba(0,0,0,0.12)">
                    ...
                    <tr>
                        <td style="padding:50px 40px">
                            <h2 style="margin:0 0 24px 0;color:#1f2937;font-size:26px;font-weight:700;letter-spacing:-0.5px">Votre chauffeur est en route !</h2>
                            <p style="margin:0 0 35px 0;color:#4b5563;font-size:16px;line-height:1.7">Bonjour,<br><br>Excellente nouvelle ! Votre chauffeur <strong style="color:#6366f1">' . htmlspecialchars($driver) . '</strong> est en route pour venir vous chercher à votre position.</p>
                            <table width="100%" cellpadding="0" cellspacing="0" style="background:#f9fafb;border-radius:12px;overflow:hidden;margin:30px 0">
                                ...
                                <tr>
                                    <td style="padding:12px;border-bottom:1px solid #e5e7eb"><strong style="color:#374151">ID course:</strong></td>
                                    <td style="padding:12px;border-bottom:1px solid #e5e7eb;text-align:right;color:#6b7280">' . htmlspecialchars($rideId) . '</td>
                                </tr>
                                <tr>
                                    <td style="padding:12px;background:#f9fafb"><strong style="color:#374151">Chauffeur:</strong></td>
                                    <td style="padding:12px;background:#f9fafb;text-align:right"><span style="font-size:18px;font-weight:700;color:#6366f1">' . htmlspecialchars($driver) . '</span></td>
                                </tr>
                            </table>
                            <div style="background:linear-gradient(135deg,#dbeafe 0%,#bfdbfe 100%);border-left:5px solid #3b82f6;padding:20px 24px;margin:30px 0;border-radius:12px">
                                <p style="margin:0;color:#1e40af;font-size:15px;line-height:1.6"><strong>⏱️ Arrivée estimée</strong><br>Le chauffeur sera à votre position dans quelques minutes. Merci de vous tenir prêt(e) !</p>
                            </div>
    ...
    $mail->AltBody = 'Votre chauffeur ' . $driver . ' est en route.';
    $mail->send();
